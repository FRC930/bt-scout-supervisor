...
export class BluetoothService {
  private _deviceStatus: BehaviorSubject<InitializeResult | null> =
    new BehaviorSubject<InitializeResult | null>(null);
  public readonly deviceStatus: Observable<InitializeResult | null> =
    this._deviceStatus.asObservable();
    ...
    this.ble.initialize().subscribe((s) => {
      ...
      this.ble
        ...
        .subscribe((result) => {
          ...
          console.log('value', value, atob(value || 'IHt9'));
  ...
  subscribeDeviceToCharacteristic(
    characteristic: string,
    deviceAddress: string
  ) {
    if (
      this.charSubscriptions[characteristic] &&
      this.charSubscriptions[characteristic].indexOf(deviceAddress) === -1
    ) {
      this.charSubscriptions[characteristic] = [
        deviceAddress,
        ...this.charSubscriptions[characteristic],
    ...
    console.log(JSON.stringify(this.charSubscriptions));
  }

  unsubscribeDeviceFromCharacteristic(
    characteristic: string,
    deviceAddress: string
  ) {
    if (
      ...
      this.charSubscriptions[characteristic].splice(
        this.charSubscriptions[characteristic].indexOf(deviceAddress),
        1
      );
    }
  }
  ...
  async notify(characteristic: string, response: string) {
    console.log('notify', btoa(response));
